...
def main(dirname, outputname, task_pool = 20):
    ...
    for gathername, sourcename in [("cell.csv", "MyExpt_cell.csv"),
        ...
        def file_process_callback(_):
            progress['i'] = progress['i'] + 1
            printProgressBar(progress['i'], progress['all'],
                             prefix=filename, length=50)
        ...
        for filename in cellfiles:
            ...
            pool.apply_async(file_processor, (filename, all_data_matrix, ),
                             callback=file_process_callback)

        while(thread_finished.value < task_pool):
            printProgressBar(thread_count.value,
                             thread_total.value,
                             prefix=("%d/%d" % (thread_count.value, thread_total.value)),
                             length=50)
            time.sleep(0.1)
